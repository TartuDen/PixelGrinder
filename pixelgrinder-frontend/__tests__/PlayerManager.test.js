...
global.Phaser = {
  ...
  describe("PlayerManager", () => {
    ...
    beforeEach(() => {
      ...
      // Mock tilemap object with findObject method
      tilemap = {
        findObject: jest.fn().mockReturnValue({ x: 100, y: 150 }),
      };
  
      // Mock scene with physics and updateUI
      mockScene = {
        physics: {
          add: {
            sprite: jest.fn(),
      ...
      // Mock physics.add.sprite to return the mocked player sprite
    ...
    test("getPlayerStats should return correct stats", () => {
      ...
      // Verify that calculatePlayerStats was called five times (one for each stat)
      expect(calculatePlayerStats).toHaveBeenCalledTimes(6); // Adjusted based on the number of stats
    ...
    test("should equip an item and update stats correctly", () => {
      ...
      expect(console.log).toHaveBeenCalledWith(
        `Player Health Initialized to Max: ${playerManager.currentHealth}`
      );
      expect(console.log).toHaveBeenCalledWith(
        `Player Mana Initialized to Max: ${playerManager.currentMana}`
      );
      expect(console.log).toHaveBeenCalledWith(
    ...
    test("should regenerate player stats correctly", () => {
      ...
      expect(consoleSpy).toHaveBeenCalledWith(
        `Regenerated +10 mana, +5 HP`
      );
      expect(consoleSpy).toHaveBeenCalledWith(
        `Regenerated +5 mana, +2 HP`
      );
