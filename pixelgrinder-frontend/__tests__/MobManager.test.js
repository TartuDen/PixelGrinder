...
function createMockMob(id = "goblin") { // Default to "goblin" which is an enemy
  return {
...
// Define mock classes for Text to simulate Phaser's GameObjects
const mockText = {
...
beforeEach(() => {
  ...
  mockGroup = {
    ...
    getChildren: jest.fn(() => [mockGroup.create.mock.results[0].value, mockGroup.create.mock.results[1].value]),
  };
...
describe("MobManager", () => {
  test("should create mobs correctly", () => {
    ...
    expect(mockScene.physics.add.group).toHaveBeenCalledWith({ collideWorldBounds: true });
    ...
    mockGroup.getChildren().forEach((mob) => {
      ...
      expect(mockScene.add.text).toHaveBeenCalledWith(mob.customData.spawnX, mob.customData.spawnY - 20, `HP: ${mob.customData.hp}`, {
        font: "14px Arial",
        fill: "#ffffff",
        shadow: {
          offsetX: 2,
  ...
  test("should handle mob death and respawn", () => {
    ...
    expect(mob.customData.hpText.setText).toHaveBeenCalledWith(`HP: ${mobsData["goblin"].health}`);
    expect(mob.customData.hpText.setPosition).toHaveBeenCalledWith(mob.customData.spawnX, mob.customData.spawnY - 20);
